...
- (BOOL)webView:(UIWebView *)webView
  ...
  if ([request.URL.host isEqual: self.originURL.host]) {
    ...
    return NO;
  } else if ([request.URL.scheme isEqual: @"http"] || [request.URL.scheme isEqual:@"http"]) {
...
- (BOOL)loadWithPlayerParams:(NSDictionary *)additionalPlayerParams {
  ...
  if (!path) {
      path = [[[self class] frameworkBundle] pathForResource:@"YTPlayerView-iframe-player" ofType:@"html"];
  }
